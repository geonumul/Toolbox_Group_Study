# -*- coding: utf-8 -*-
"""실내디자인시공과실무 정리 슬라이드 공용 도우미.

build_slides_w2.py, build_slides_w3.py 가 불러 쓴다.
규칙: work\\_rules\\00_공통_원칙.md, work\\_rules\\정리슬라이드_작성규칙.md,
      work\\interior-construction\\rules\\과목정보.md

쓰는 법:

    import os, sys
    sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
    from icslide_lib import *

    U = []
    U.append(unit("w2-1", "시공의 뜻과 공사관계자", "이 단원을 마치면 ... 할 수 있어요",
                  ["발주자", "감리자", "시공자"],
                  [title("공사의 등장인물", "..."), goal("...", "..."), ..., recap("...")]))
    write(out("2"), "2", "2강. 실내건축시공실무 총론, 가설공사, 철거공사", U)

- 전문용어는 t("메탈 스터드") 로 써서 **메탈 스터드(Metal Stud)** 모양을 만든다. 표기는
  work\\interior-construction\\terms\\<주차>.json 을 그대로 쓴다(새로 만들지 않는다).
- 단원 제목은 문제은행(bank\\l23.json)의 unit 문자열에서 번호를 뗀 뒷부분과 똑같아야 한다.
- 숫자와 치수는 슬라이드, 정리노트, tips.html 에 있는 것만 쓴다. 지어내지 않는다.
...
# 과목 전체에서 되풀이하는 주문
MANTRA1 = "사람 → 방식 → 입찰 → 계획 → 관리 → 준공"
MANTRA2 = "가설 → 철거 → 경량철골 → 마감"
MANTRA3 = "재노외경 → 간접 → 현장 → 일반 → 이윤"
MANTRA4 = "먹, 아래, 시작, 위, 기둥 으로 뼈대 → 보드, 솜, 보드, 보드 로 살"
MANTRA5 = "위에서 아래로 매달기"

# 금지 문자(긴 줄표, 짧은 줄표, 가운뎃점 두 가지). 코드 번호로 적어 이 파일에 글자를 넣지 않는다.
...
# ---------------------------------------------------------------- 문제은행 단원 이름 (한 줄로 맞추기)
def _bank_tails():
    """{주차: {단원 제목: {문제은행 번호}}}. unit 문자열에서 '2-3 ' 같은 번호를 뗀다."""
    out = {}
...
def stp(head, given, steps, answer):
    """시공 순서, 계산 순서를 한 단계씩. 순서 문제가 나오는 단원은 반드시 넣는다."""
...
def eng(head, en, ko, tip=None):
    """english 는 '외워 쓸 답안 문장'. en 에 시험지에 그대로 옮겨 쓸 한국어 문장."""
...
def HAT(x, y, w, h, lines, c="box", b=0):
    """왼쪽 정렬 상자. lines: [(글자, 크기, 클래스), ...]"""
    out = [R(x, y, w, h, c, b)]
    total = sum(sz for _, sz, _ in lines) + 5 * (len(lines) - 1)
    assert total <= h - 4, "상자 높이가 모자라요 %r" % (lines,)
    cy = y + (h - total) / 2
    for s, sz, cl in lines:
        cy += sz
        out.append(TX(x + 5, round(cy - 2), s, cl, b, sz, "start", maxw=w - 10))
        cy += 5
    return "".join(out)
...
def unit(uid, ttl, gl, terms, slides, recall=()):
    ...
    assert num in tails[ttl], ("%s: 문제은행은 %r 을 %s번으로 부릅니다."
                               % (uid, ttl, ", ".join(str(x) for x in sorted(tails[ttl]))))
    ...
    assert len(recall) >= 2 or uid == "w2-1", "%s: 앞 단원 용어를 2개 이상 다시 불러요" % uid
...
def write(path, week, ttl, units):
    ...
    assert "정수연" not in txt, "교수님 이름을 쓰지 않아요"
    assert "교수님" not in txt, "녹음이 없으므로 교수님 말을 옮기지 않아요"
