...
SVG_CNN = (
    ...
    + '<text class="tm b2" x="130" y="248" font-size="13" text-anchor="middle">폭 3 필터가 한 칸씩 내려가요</text>'
...
U.append({"id": "w3-4", "title": "기울기, 야코비안, 연쇄 법칙", "goal": "미분에서 야코비안까지 이어지는 생각을 잡고, 연쇄 법칙을 숫자로 쓸 수 있어요.", "terms": t, "slides": [
    ...
    steps("손계산: 정말 그런지 밀어 보기 (N3 p.18)", [
    ...
    ], "12.006 은 공식이 준 12 와 거의 같아요", "**미분(Derivative)** 은 딱 한 질문이에요. x 를 살짝 밀면 f 는 얼마나 움직일까요"),
...
U.append({"id": "w3-5", "title": "손으로 푸는 역전파: δ 와 shape", "goal": "δ 를 한 번 구해 세 기울기를 모두 쓰고, shape 으로 식을 검사할 수 있어요.", "terms": t, "slides": [
    ...
    recap("**순전파(Forward Pass)**: x = [1,2,3] → z = [4,-4] → h = [4,0] → s = 8",
          ...
          "오늘의 용어: 오차 신호, 연쇄 법칙, 외적(Outer Product), 전치(Transpose), 모양(Shape), 임베딩, 역전파"),
]})

# ---------------------------------------------------------------- w3-6
t = [T("계산 그래프", "Computation Graph", "계산을 노드와 화살표로 그린 그림이에요.",
       "입력은 시작 노드, 더하기나 곱하기 같은 연산은 중간 노드예요. PyTorch 가 자동으로 만들어 줘요."),
     T("순전파", "Forward Pass", "입력에서 출력 쪽으로 계산을 차례로 해 나가는 것이에요."),
     T("국소 기울기", "Local Gradient", "노드 하나가 자기 입력과 출력만 보고 구한 미분이에요."),
     T("상류 기울기", "Upstream Gradient", "내 출력 쪽에서 나에게 내려온 기울기예요."),
     T("자동 미분", "Automatic Differentiation (Autodiff)", "프로그램이 계산 그래프를 따라 미분을 자동으로 해 주는 기능이에요."),
     T("기울기 검사", "Gradient Checking", "손으로 구한 기울기를 수치 기울기와 비교해 확인하는 것이에요."),
     T("수치 기울기", "Numeric Gradient", "값을 아주 조금 올리고 내려서 생긴 차이로 어림한 기울기예요.")]
U.append({"id": "w3-6", "title": "계산 그래프와 자동 미분", "goal": "그래프를 거꾸로 걸으며 기울기를 구하고, 기울기 검사를 할 수 있어요.", "terms": t, "slides": [
    title("그래프를 거꾸로 걷기", "노드 하나만 알면 돼요"),
    goal("**계산 그래프(Computation Graph)** 와 **순전파(Forward Pass)**", "하류 = **상류 기울기(Upstream Gradient)** 곱하기 **국소 기울기(Local Gradient)**", "**자동 미분(Automatic Differentiation (Autodiff))** 과 **기울기 검사(Gradient Checking)**"),
    fig("풀이 예를 그래프로 (N3 p.27)", SVG_GRAPH, "x, W 를 곱하고 b 를 더하고 f 를 씌우고 u 와 곱하면 점수 8 이 나와요", 2),
    pts("그래프로 보면 (N3 p.27)", "컴퓨터는 신경망을 **계산 그래프(Computation Graph)** 로 봐요",
        "시작 노드는 입력값(x, W, b, u), 안쪽 노드는 연산(곱하기, 더하기, f)이에요",
        "왼쪽에서 오른쪽으로 값을 차례로 구하는 것이 **순전파(Forward Pass)** 예요",
        "중간값을 모두 저장해 둬요. 거꾸로 갈 때 다시 쓰거든요"),
    steps("손계산: 그래프를 거꾸로 걷기 (N3 p.28)", [
        "출발: s 를 s 로 미분하면 1 이에요",
        "점곱 노드: ∂s/∂h = u = [2, 3]",
        "f 노드: ReLU 미분 [1, 0] 을 같은 자리끼리 곱해 δ = [2, 0]",
        "더하기 노드: b 쪽으로 그대로 → ∂s/∂b = [2, 0]",
        "곱하기 노드: W 쪽은 [[2, 4, 6], [0, 0, 0]], x 쪽은 [2, 0, 2]",
    ], "앞 단원에서 손으로 구한 값과 똑같이 나와요", "**역전파(Backpropagation)** 는 그래프 위의 **연쇄 법칙(Chain Rule)** 이에요"),
    form("노드 하나의 규칙 (N3 p.29)", r"\frac{\partial J}{\partial x} = \frac{\partial J}{\partial y}\cdot\frac{\partial y}{\partial x}",
         [(r"\frac{\partial J}{\partial y}", "**상류 기울기(Upstream Gradient)**: 뒤에서 받은 것"),
          (r"\frac{\partial y}{\partial x}", "**국소 기울기(Local Gradient)**: 노드 안에서 구한 것"),
          (r"\frac{\partial J}{\partial x}", "하류 기울기: 앞으로 보낼 것")],
         "받은 것 곱하기 내 것 = 보낼 것. 어떤 노드도 전체 네트워크를 볼 필요가 없어요."),
    steps("손계산: 제곱 노드 하나", [
        "노드는 y = x², 들어온 값 x = 3, 나간 값 y = 9",
        "뒤에서 **상류 기울기(Upstream Gradient)** 2 가 도착했어요",
        "**국소 기울기(Local Gradient)**: y = x² 의 미분 2x = 6",
        "하류 기울기: 2 x 6 = 12 를 앞으로 보내요",
    ], "∂J/∂x = 12. 이 노드는 J 가 무엇인지 몰라도 계산할 수 있어요", "이 성질 덕분에 **자동 미분(Automatic Differentiation (Autodiff))** 도구를 만들 수 있어요"),
    ana("배관공처럼 읽기", "물(기울기)이 파이프를 거꾸로 흘러요. 갈림길에서 나뉘고, 밸브에서 한쪽만 열리고, 스위치에서 굵기가 바뀌어요.",
        [("양쪽에 똑같이 보냄", "더하기 노드"), ("한쪽 밸브만 열림", "max 노드"), ("상대편 굵기로 바뀜", "곱하기 노드"), ("두 파이프가 만나면 합쳐짐", "갈라졌던 길의 기울기는 더해요")]),
    fig("세 노드, 세 성격 (N3 p.30)", SVG_NODES, "더하기는 나눠 주고, max 는 이긴 쪽에만, 곱하기는 상대 값을 곱해요", 3),
    steps("손계산: 세 성격을 한 그래프에서", [
        "f = (a + b) x max(b, c), a = 1, b = 3, c = 2 라고 해요",
        "**순전파(Forward Pass)**: q = a + b = 4, r = max(b, c) = 3, f = q x r = 12",
        "출발 1. 곱하기 노드는 맞바꿔요: q 쪽 1 x 3 = 3, r 쪽 1 x 4 = 4",
        "더하기 노드는 나눠 줘요: a 에 3, b 에 3",
        "max 노드는 이긴 쪽(b = 3)에만 4, c 에는 0. b 는 두 곳에 쓰였으니 3 + 4 = 7",
    ], "∂f/∂a = 3, ∂f/∂b = 7, ∂f/∂c = 0", "갈라졌던 길의 기울기는 더해요"),
    pts("전체 알고리즘과 그 값 (N3 p.31)", "앞으로 한 번 걸으며 값을 구해 저장해요",
        "뒤로 한 번 걸으며 **상류 기울기(Upstream Gradient)** 곱하기 **국소 기울기(Local Gradient)** 를 해요",
        "갈림길에서는 더해요. 뒤로 한 번의 비용이 앞으로 한 번과 비슷해요",
        "파라미터 100 만 개를 하나씩 흔들면 순전파 100 만 번, 1 초씩이면 약 11 일이에요",
        "**역전파(Backpropagation)** 는 앞 한 번 + 뒤 한 번, 곧 약 2 초예요"),
    form("수치 기울기 (N3 p.32)", r"f'(\theta) \approx \frac{f(\theta + h) - f(\theta - h)}{2h}, \quad h \approx 10^{-4}",
         [(r"f(\theta + h)", "θ 를 아주 조금 올려서 계산한 값"),
          (r"f(\theta - h)", "θ 를 아주 조금 내려서 계산한 값"),
          (r"2h", "올린 곳과 내린 곳 사이 거리"),
          (r"h \approx 10^{-4}", "0.0001. 은닉층 값 h 와는 다른 아주 작은 수예요")],
         "양쪽으로 살짝 밀어 본 차이를 거리로 나누면 **수치 기울기(Numeric Gradient)** 예요."),
    steps("손계산: 기울기 검사 (N3 p.32)", [
        "f(θ) = θ³ + 2θ, θ = 2 에서 확인해요",
        "손 공식: f'(θ) = 3θ² + 2 = 12 + 2 = 14",
        "h = 0.0001 로 양쪽을 밀어 f(2.0001) 과 f(1.9999) 를 계산해요",
        "차이를 2h = 0.0002 로 나누면 약 14.0",
        "쉬운 예도: f(θ) = θ², θ = 3, h = 0.1 이면 (9.61 - 8.41) ÷ 0.2 = 6",
    ], "**수치 기울기(Numeric Gradient)** 와 손 공식이 같아요. **기울기 검사(Gradient Checking)** 통과", "수치 기울기는 느려서 학습이 아니라 검사에만 써요"),
    cmp_("기울기를 구하는 세 가지 길 (N3 p.32)", ["방법", "속도", "쓰임"],
         [["손으로 유도", "사람이 느려요", "원리 이해, 과제 2"],
          ["**자동 미분(Automatic Differentiation (Autodiff))**", "빨라요", "실제 학습"],
          ["**수치 기울기(Numeric Gradient)**", "아주 느려요", "맞는지 검사"]]),
    check("**역전파(Backpropagation)** 에서 **순전파(Forward Pass)** 값을 저장하는 이유는?",
          ["거꾸로 갈 때 **국소 기울기(Local Gradient)** 를 구하는 데 필요해서", "출력을 두 번 보여 주려고", "학습률을 정하려고", "메모리를 아끼려고"], 0,
          "예를 들어 ReLU 미분은 z 의 부호를 알아야 하고, 곱하기 노드는 상대 입력값이 필요해요."),
    check("한 값이 **순전파(Forward Pass)** 에서 두 곳에 쓰였다면 역전파에서 그 값의 기울기는?",
          ["두 기울기의 합", "두 기울기 중 큰 것", "두 기울기의 곱", "0"], 0, "갈라진 가지는 되돌아올 때 더해요."),
    warn("헷갈리기 쉬운 점", "loss.backward() 는 기울기만 채워요. 파라미터를 바꾸는 것은 옵티마이저 단계예요.",
         "**수치 기울기(Numeric Gradient)** 식의 h 는 0.0001 같은 아주 작은 수예요. 은닉층 값 h 와 글자만 같아요.",
         "상류는 출력 쪽, 하류는 입력 쪽이에요. 기울기가 거꾸로 흐르니 방향도 뒤에서 앞이에요.",
         "속도를 얻는 대신 메모리를 써요. 학습이 추론보다 메모리를 훨씬 많이 먹어요."),
    recap("**계산 그래프(Computation Graph)**: 앞으로 값, 뒤로 기울기",
          "노드 하나의 규칙: 하류 = **상류 기울기(Upstream Gradient)** x **국소 기울기(Local Gradient)**",
          "더하기는 나눠 주고, max 는 이긴 쪽만, 곱하기는 맞바꿔요. **기울기 검사(Gradient Checking)** 로 확인해요",
          "오늘의 용어: 계산 그래프, 순전파, 국소 기울기, 상류 기울기, 자동 미분, 기울기 검사, 수치 기울기"),
]})

# ---------------------------------------------------------------- w3-7
t = [T("언어 모델", "Language Model (LM)", "휴대폰 자판의 다음 단어 추천처럼 다음 토큰을 맞히는 모델이에요."),
     T("다음 토큰 예측", "Next Token Prediction", "지금까지 나온 토큰으로 바로 다음 토큰을 맞히는 일이에요.",
       "교수님: GPT 도 다 결국 이거예요. 이것이 거의 모든 걸 결정해요."),
     T("엔그램", "n-gram", "바로 붙어 있는 단어 n 개 묶음이에요."),
     T("마르코프 가정", "Markov Assumption", "다음 단어는 바로 앞 n-1 개 단어에만 달려 있다고 치는 가정이에요."),
     T("말뭉치", "Corpus", "횟수를 세는 큰 글 모음이에요."),
     T("희소성 문제", "Sparsity Problem", "셀 묶음이 거의 안 나와서 횟수가 0 투성이인 문제예요."),
     T("어휘", "Vocabulary", "모델이 아는 토큰 종류의 목록이에요.")]
U.append({"id": "w3-7", "title": "언어 모델과 n-gram", "goal": "언어 모델이 내는 것이 무엇인지 말하고, n-gram 확률을 손으로 구할 수 있어요.", "terms": t, "slides": [
    title("다음 단어 맞히기", "언어 모델과 세어서 맞히기"),
    goal("**언어 모델(Language Model (LM))** 이 내는 것", "**엔그램(n-gram)** 확률 손계산", "세기가 부딪히는 세 벽"),
    ana("휴대폰 자판의 다음 단어 추천", "휴대폰에 '오늘 수업은 몇 시에' 까지 치면 자판 위에 '끝나요', '시작해요' 같은 추천이 떠요. 앞 글을 보고 후보마다 가능성을 매긴 거예요.",
        [("자판의 추천 단어", "다음 단어 후보"), ("추천 순서", "후보마다 붙은 확률"), ("추천 기능 자체", "**언어 모델(Language Model (LM))**")]),
    fig("후보마다 확률을 붙여요 (N3 p.35)", SVG_LM, "the lecture will end at 뒤에 9pm 0.2, noon 0.1, midnight 0.05, 나머지 0.65", 3),
    form("언어 모델이 내는 것 (N3 p.35)", r"P\left(x^{(t+1)} \mid x^{(t)}, \ldots, x^{(1)}\right)",
         [(r"x^{(1)}, \ldots, x^{(t)}", "지금까지 나온 토큰 t 개"),
          (r"x^{(t+1)}", "바로 다음에 올 토큰. 맞히려는 것"),
          (r"\mid", "'~가 주어졌을 때' 라고 읽어요"),
          (r"P(\cdot)", "**어휘(Vocabulary)** 의 후보마다 이 값을 내면 확률 분포가 돼요")],
         "앞 토큰들이 주어졌을 때 다음 토큰의 확률을 후보마다 내는 것이 **언어 모델(Language Model (LM))** 이에요. 이것이 **다음 토큰 예측(Next Token Prediction)** 이에요."),
    pts("매일 쓰고 있어요 (N3 p.36)", "휴대폰 자판 자동 완성, 검색어 추천",
        "음성 인식과 번역: 비슷한 후보 중 더 그럴듯한 쪽 고르기",
        "ChatGPT: 아주 큰 **언어 모델(Language Model (LM))** 이 다음 토큰을 거듭 뽑는 것",
        "LLM 의 가운데 두 글자 LM 이 바로 오늘의 주제예요"),
    ana("바로 앞 몇 단어만 보고 맞히기", "기억력이 짧은 친구가 끝말을 이어요. 바로 앞 세 단어만 기억하고, 예전에 그 뒤에 무엇이 몇 번 나왔는지 떠올려 가장 많은 것을 말해요.",
        [("바로 앞 세 단어만 기억", "**마르코프 가정(Markov Assumption)**"), ("세 단어 + 다음 단어 묶음", "4-gram, 곧 **엔그램(n-gram)**"), ("몇 번 나왔나 떠올리기", "**말뭉치(Corpus)** 에서 횟수 세기")]),
    form("세어서 확률 구하기 (N3 p.37)", r"P(w \mid \text{students opened their}) \approx \frac{\mathrm{count}(\text{students opened their } w)}{\mathrm{count}(\text{students opened their})}",
         [(r"w", "다음에 올 후보 단어 하나"),
          (r"\mathrm{count}(\ldots w)", "분자: 4단어 묶음이 **말뭉치(Corpus)** 에 나온 횟수"),
          (r"\mathrm{count}(\ldots)", "분모: 앞 3단어 묶음이 나온 횟수")],
         "앞 3단어 뒤에 w 가 온 횟수를 앞 3단어가 나온 횟수로 나눠요. 학습이 아니라 세기예요."),
    fig("버려진 문맥 (N3 p.38)", SVG_NGRAM, "**마르코프 가정(Markov Assumption)** 으로 proctor 와 clock 이 버려지고 앞 3단어만 남아요", 3),
    steps("손계산: 슬라이드 숫자 그대로 (N3 p.38)", [
        "문장: as the proctor started the clock, the students opened their ___",
        "4-gram 이라 앞 3단어 students opened their 만 조건으로 봐요",
        "분모: count(students opened their) = 1,000",
        "books 는 400 번 → 400 ÷ 1,000 = 0.40",
        "exams 는 100 번 → 100 ÷ 1,000 = 0.10",
    ], "모델은 books(0.40) 를 exams(0.10) 보다 그럴듯하다고 봐요", "하지만 정답에 가까운 단서 proctor, clock 은 이미 버려졌어요"),
    pts("모델이 틀린 게 아니에요 (N3 p.38)", "감독관이 시계를 켰으니 사람은 exams 를 떠올려요",
        "**엔그램(n-gram)** 은 **마르코프 가정(Markov Assumption)** 때문에 그 단서를 볼 수 없었어요",
        "슬라이드 말로 중요한 단어를 잊도록 강요당한 거예요",
        "그래서 '앞 문맥을 계속 기억하며 읽을 수 없을까' 가 다음 이야기예요"),
    cmp_("세기가 부딪히는 벽 (N3 p.39)", ["벽", "내용", "고치기"],
         [["**희소성 문제(Sparsity Problem)**", "분자를 못 봤으면 확률이 0", "스무딩"],
          ["**희소성 문제(Sparsity Problem)**", "분모를 못 봤으면 조건을 걸 수조차 없어요", "더 짧은 문맥으로 백오프"],
          ["저장", "본 **엔그램(n-gram)** 마다 횟수를 적어 둬야 해요", "없음"],
          ["유사도 없음", "opened their 와 opened her 가 횟수를 안 나눠요", "임베딩(다음 단원)"]]),
    steps("손계산: n 이 커지면 왜 나빠지나", [
        "**어휘(Vocabulary)** 가 1만 개라고 해요",
        "앞 1단어 문맥의 종류: 1만 가지",
        "2단어 묶음의 종류: 1만 x 1만 = 1억 가지",
        "3단어 묶음: 1만의 세제곱 = 1조 가지",
        "**말뭉치(Corpus)** 가 아무리 커도 1조 가지를 다 볼 수는 없어요",
    ], "n 이 1 늘 때마다 조합이 **어휘(Vocabulary)** 크기 배로 늘어요. 실제로는 n 이 5 이하예요", "어휘 1만 개는 설명을 위한 숫자예요"),
    steps("손계산: n-gram 으로 글 쓰기 (N3 p.40)", [
        "trigram 이라 앞 2단어만 보고 뽑아요. 시작은 today the",
        "today the → price 0.31, the price → of 0.62, price of → gold 0.57",
        "of gold → is 0.34, gold is → shiny 0.41",
        "이 경로의 확률: 0.31 x 0.62 = 0.1922, x 0.57 = 0.1096",
        "x 0.34 = 0.0372, x 0.41 = 0.0153",
    ], "today the price of gold is shiny 가 나오고 확률은 약 0.0153", "가까이 보면 문법이 맞는데 전체로는 앞뒤가 안 맞아요"),
    check("4-gram 모델은 다음 단어를 맞힐 때 앞의 몇 단어를 보나요?",
          ["3단어", "4단어", "문장 전체", "2단어"], 0, "**엔그램(n-gram)** 은 앞 n-1 개를 봐요. 묶음 전체가 4단어예요."),
    check("**엔그램(n-gram)** 언어 모델이 확률을 구하는 방법은?",
          ["**말뭉치(Corpus)** 에서 묶음이 나온 횟수를 세어 나눠요", "경사 하강법으로 학습해요", "사람이 직접 적어요", "무작위로 정해요"], 0,
          "슬라이드 말로 Then just COUNT in a big corpus. 학습이 아니라 세기예요."),
    warn("헷갈리기 쉬운 점", "분자가 0 이면 스무딩, 분모가 0 이면 백오프예요. 짝을 바꿔 외우지 마세요.",
         "**엔그램(n-gram)** 의 n 은 묶음 전체 크기예요. 조건으로 보는 앞 단어는 n-1 개예요.",
         "n 을 키우면 문맥은 길어지지만 **희소성 문제(Sparsity Problem)** 와 저장이 더 나빠져요.",
         "확률의 연쇄 법칙은 확률을 곱으로 쪼개는 규칙이에요. 2부 미분의 연쇄 법칙과 이름만 같아요."),
    recap("**언어 모델(Language Model (LM))** = 앞 토큰으로 다음 토큰의 확률 분포 내기, 곧 **다음 토큰 예측(Next Token Prediction)**",
          "**엔그램(n-gram)** 은 **마르코프 가정(Markov Assumption)** 으로 앞 n-1 개만 보고 **말뭉치(Corpus)** 에서 세요",
          "세 벽: **희소성 문제(Sparsity Problem)**, 저장, 유사도 없음. 400/1000 = 0.40, 100/1000 = 0.10",
          "오늘의 용어: 언어 모델(LM), 다음 토큰 예측, n-gram, 마르코프 가정, 말뭉치, 희소성 문제, 어휘"),
]})

# ---------------------------------------------------------------- w3-8
t = [T("퍼플렉서티", "Perplexity (PPL)", "다음 단어를 고를 때 헷갈리는 후보가 평균 몇 개인지 나타낸 숫자예요.",
       "PPL = exp(교차 엔트로피 손실)이에요. 낮을수록 좋고, 최저는 1, 균등하면 어휘 크기 V 예요."),
     T("교차 엔트로피", "Cross-Entropy", "정답 단어에 준 확률의 -log 를 벌점으로 쓰는 손실이에요."),
     T("손실 함수", "Loss Function", "모델이 틀린 정도를 매기는 벌점이에요."),
     T("어휘", "Vocabulary", "모델이 아는 토큰 종류의 목록이에요."),
     T("토크나이저", "Tokenizer", "글을 토큰으로 자르는 도구예요."),
     T("언어 모델", "Language Model (LM)", "앞 글을 보고 다음 토큰의 확률을 내는 모델이에요.")]
U.append({"id": "w3-8", "title": "언어 모델 재기: 퍼플렉서티", "goal": "퍼플렉서티를 두 가지 방법으로 계산하고, 언제 비교가 의미 없는지 말할 수 있어요.", "terms": t, "slides": [
    title("퍼플렉서티", "헷갈리는 후보가 평균 몇 개"),
    goal("**퍼플렉서티(Perplexity (PPL))** 가 **언어 모델(Language Model (LM))** 의 무엇을 재는지", "**교차 엔트로피(Cross-Entropy)** 와의 관계", "언제 비교가 의미 없어지는지"),
    ana("헷갈리는 후보가 평균 몇 개", "퀴즈 보기가 2개면 덜 헷갈리고 100개면 많이 헷갈려요. **언어 모델(Language Model (LM))** 이 평균 몇 개 보기 사이에서 헷갈리는지를 숫자 하나로 나타낸 거예요.",
        [("퀴즈 보기 수", "**퍼플렉서티(Perplexity (PPL))**"), ("보기가 적을수록 쉬움", "낮을수록 좋은 모델"), ("보기 1개", "PPL = 1, 완벽한 모델")]),
    fig("퍼플렉서티 눈금 (N3 p.41, p.42)", SVG_PPL, "완벽하면 1, 슬라이드 예제는 약 5.7, 아무것도 모르면 **어휘(Vocabulary)** 크기", 3),
    form("퍼플렉서티 식 (N3 p.41)", r"\mathrm{PPL} = \exp(J) = \prod_{t=1}^{T} \left( \frac{1}{P\left(x^{(t+1)} \mid x^{(\le t)}\right)} \right)^{1/T}",
         [(r"P(x^{(t+1)} \mid x^{(\le t)})", "실제 정답 다음 단어에 모델이 준 확률"),
          (r"\frac{1}{P}", "그 확률의 역수. 확률이 1/4 이면 4 예요"),
          (r"\prod_{t=1}^{T}", "T 개 위치를 모두 곱하기"),
          (r"(\cdot)^{1/T}", "T 제곱근, 곧 기하 평균"),
          (r"J", "**교차 엔트로피(Cross-Entropy)** 의 평균, 곧 **손실 함수(Loss Function)**")],
         "정답 확률의 역수를 기하 평균한 것이 **퍼플렉서티(Perplexity (PPL))** 이고, 이는 평균 손실 J 에 exp 를 씌운 것과 같아요."),
    form("J: 위치마다의 벌점을 평균 내기 (N3 p.41)", r"J = \frac{1}{T} \sum_{t=1}^{T} -\log P\left(x^{(t+1)} \mid x^{(\le t)}\right)",
         [(r"-\log P", "정답 단어 확률의 음의 로그. 한 위치의 **교차 엔트로피(Cross-Entropy)** 벌점"),
          (r"\sum_{t=1}^{T}", "T 개 위치의 벌점을 모두 더하기"),
          (r"\frac{1}{T}", "위치 수로 나눠 평균"),
          (r"J", "평균 벌점. 학습이 줄이는 바로 그 **손실 함수(Loss Function)**")],
         "학습 때 줄이는 손실에 exp 만 씌우면 **퍼플렉서티(Perplexity (PPL))** 예요. 따로 계산할 게 거의 없어요."),
    steps("손계산 1: 아무것도 모르는 모델", [
        "**어휘(Vocabulary)** V = 10,000, 모든 단어에 똑같이 1/10,000 을 줬다고 해요",
        "위치마다 벌점 -log(1/10,000) = log 10,000 ≈ 9.21",
        "모든 위치가 같으니 평균 J 도 9.21",
        "PPL = exp(9.21) = 10,000",
    ], "균등하게 찍으면 PPL 은 **어휘(Vocabulary)** 크기 V 와 같아요", "자연로그를 써요"),
    steps("손계산 2: 슬라이드 예제 (N3 p.42)", [
        "모델이 the, cat, sat, down 에 1/2, 1/4, 1/8, 1/16 을 줬어요",
        "정답 확률의 역수: 2, 4, 8, 16",
        "모두 곱하기: 2 x 4 x 8 x 16 = 1,024",
        "단어가 4 개라 네제곱근: 1,024 의 네제곱근",
        "1,024 는 2 의 10제곱이니 2 의 2.5제곱 ≈ 5.66",
    ], "PPL ≈ 5.7, 곧 약 6개 후보 중에서 고르는 정도로 헷갈려요", "the 1/2, cat 1/4, sat 1/8, down 1/16"),
    steps("손계산 3: 같은 답을 exp(J) 로", [
        "위치마다 벌점 -log P: log 2, log 4, log 8, log 16",
        "합 = log(2 x 4 x 8 x 16) = log 1,024",
        "평균 J = log 1,024 ÷ 4 ≈ 1.733",
        "PPL = exp(1.733) ≈ 5.66",
    ], "두 길이 같은 답을 줘요. 로그가 곱을 합으로 바꾸기 때문이에요", "역수의 기하 평균 = exp(평균 -log P)"),
    pts("기준점 세 개 (N3 p.42)", "모든 단어에 똑같은 확률을 주면 PPL = **어휘(Vocabulary)** 크기 V",
        "정답에 늘 확률 1 을 주면 PPL = 1, 이것이 가장 낮은 값이에요",
        "PPL 이 1 보다 작을 수는 없어요. 확률이 1 을 넘을 수 없으니 역수가 1 이상이에요",
        "교수님: 낮으면 낮을수록 좋아요"),
    pts("비교할 때의 조건 (N3 p.42)", "교수님: 같은 평가 데이터, 같은 **어휘(Vocabulary)**, 같은 **토크나이저(Tokenizer)** 여야 해요",
        "한 모델은 글자 단위, 다른 모델은 서브워드 단위면 숫자를 그대로 비교할 수 없어요",
        "PPL 은 예측 단위 하나당 값이라서 그래요",
        "논문에서 PPL 을 볼 때는 어떤 단위, 어떤 **토크나이저(Tokenizer)** 인지부터 물어요"),
    check("**교차 엔트로피(Cross-Entropy)** 평균 J 와 **퍼플렉서티(Perplexity (PPL))** 의 관계는?",
          ["PPL = exp(J)", "PPL = log(J)", "PPL = 1/J", "PPL = J"], 0, "슬라이드 말로 Perplexity = exp(cross-entropy loss) 예요."),
    check("정답 확률이 네 번 모두 1/4 이면 **퍼플렉서티(Perplexity (PPL))** 는?",
          ["4", "16", "256", "1"], 0, "역수 4 를 네 번 곱하고 네제곱근을 하면 4 예요."),
    check("완벽한 **언어 모델(Language Model (LM))** 의 퍼플렉서티는?",
          ["1", "0", "**어휘(Vocabulary)** 크기", "무한대"], 0, "1 을 몇 번 곱해도 1 이에요. PPL 의 최솟값은 1 이에요."),
    warn("헷갈리기 쉬운 점", "정확도는 높을수록 좋지만 **퍼플렉서티(Perplexity (PPL))** 는 낮을수록 좋아요.",
         "기하 평균이에요. (2 + 4 + 8 + 16) ÷ 4 = 7.5 로 계산하면 틀려요.",
         "글자 모델의 PPL 27 이 서브워드 모델의 PPL 299 보다 좋다는 뜻이 아니에요. 단위가 달라요."),
    recap("**퍼플렉서티(Perplexity (PPL))** = exp(평균 **교차 엔트로피(Cross-Entropy)**), 낮을수록 좋아요",
          "1/2, 1/4, 1/8, 1/16 이면 1,024 의 네제곱근 ≈ 5.7. 균등하면 V, 완벽하면 1",
          "같은 데이터, 같은 **어휘(Vocabulary)**, 같은 **토크나이저(Tokenizer)** 여야 비교할 수 있어요",
          "오늘의 용어: 퍼플렉서티(PPL), 교차 엔트로피, 손실 함수, 어휘, 토크나이저, 언어 모델(LM)"),
]})

# ---------------------------------------------------------------- w3-9
t = [T("고정 윈도우 신경망 언어 모델", "Fixed-window Neural LM", "앞의 정해진 몇 단어만 임베딩으로 바꿔 신경망에 넣고 다음 단어를 맞히는 모델이에요."),
     T("임베딩", "Embedding", "단어의 지도 좌표. 뜻이 비슷하면 가까이 살아요."),
     T("순환 신경망", "Recurrent Neural Network (RNN)", "한 단어씩 읽으며 메모장에 요약을 고쳐 쓰는 신경망이에요.",
       "매 걸음 같은 가중치를 다시 써요. 그래서 문장이 길어져도 파라미터 수가 늘지 않아요."),
     T("은닉 상태", "Hidden State", "RNN 이 지금까지 읽은 내용을 적어 두는 메모장이에요."),
     T("파라미터", "Parameter", "학습으로 정해지는 숫자들이에요."),
     T("희소성 문제", "Sparsity Problem", "셀 묶음이 거의 안 나와서 횟수가 0 투성이인 문제예요.")]
U.append({"id": "w3-9", "title": "고정 윈도우 신경망 LM 과 RNN", "goal": "임베딩이 허문 두 벽과 RNN 이 허문 마지막 벽을 말할 수 있어요.", "terms": t, "slides": [
    title("기억을 가진 독자", "고정 윈도우에서 RNN 으로"),
    goal("**고정 윈도우 신경망 언어 모델(Fixed-window Neural LM)** 이 허문 두 벽", "**순환 신경망(Recurrent Neural Network (RNN))** 의 핵심 생각", "왜 문장이 길어도 **파라미터(Parameter)** 가 안 늘어나는지"),
    fig("창 분류기를 어휘 전체로 (N3 p.44)", SVG_FWLM, "앞 네 단어의 **임베딩(Embedding)** 을 이어 붙여 다음 단어를 맞혀요", 3),
    form("고정 윈도우 신경망 LM (N3 p.44)", r"e = [e_1; e_2; e_3; e_4],\quad h = f(We + b),\quad \hat{y} = \mathrm{softmax}(Uh)",
         [(r"e_i", "i 번째 단어의 **임베딩(Embedding)**"),
          (r"[\,\cdot\,;\,\cdot\,]", "옆으로 이어 붙이기"),
          (r"h = f(We + b)", "앞에서 본 **뉴런(Neuron)** 층 하나"),
          (r"\hat{y}", "**어휘(Vocabulary)** 전체에 대한 확률")],
         "w3-1 의 창 분류기와 똑같은데, 고르는 칸이 세 개가 아니라 **어휘(Vocabulary)** 전체예요."),
    steps("손계산: 출력 확률 읽기 (N3 p.44)", [
        "슬라이드 출력: books 0.42, presents 0.21, laptops 0.09",
        "세 개를 더하면 0.42 + 0.21 + 0.09 = 0.72",
        "확률 분포는 다 더해 1 이어야 하니 나머지 **어휘(Vocabulary)** 가 1 - 0.72 = 0.28 을 나눠 가져요",
        "**엔그램(n-gram)** 과 달리 한 번도 못 본 조합도 말이 되는 확률을 받아요",
    ], "나머지 후보의 몫은 0.28 이에요", "슬라이드 오른쪽 위에 적힌 세 숫자를 그대로 썼어요"),
    cmp_("성적표 (N3 p.45)", ["", "내용"],
         [["좋아진 점", "**희소성 문제(Sparsity Problem)** 가 없어요. 못 본 조합도 확률을 받아요"],
          ["좋아진 점", "거대한 횟수 표가 필요 없어요. **임베딩(Embedding)** 이 비슷함을 담아요"],
          ["남은 문제", "창이 여전히 고정이라 그 밖의 문맥은 버려져요"],
          ["남은 문제", "자리마다 W 의 다른 부분을 써서 the 가 1번 자리와 3번 자리에서 따로 학습돼요"]]),
    pts("원하는 두 가지 (N3 p.45)", "1 어떤 길이든 받을 수 있을 것",
        "2 자리마다 같은 방식으로 처리할 것",
        "이 둘을 주는 것이 순환(recurrence), 곧 **순환 신경망(Recurrent Neural Network (RNN))** 이에요",
        "세 벽 중 둘은 무너졌고 고정 창 하나가 남았어요"),
    ana("메모장에 요약을 고쳐 쓰는 사람", "한 단어씩 읽으면서 옆의 메모장에 지금까지의 요약을 계속 고쳐 써요. 다음 단어를 맞힐 때는 그 메모장만 보면 돼요.",
        [("한 단어씩 읽기", "시점마다 입력 하나"), ("메모장", "**은닉 상태(Hidden State)**"), ("요약을 고쳐 쓰기", "새 **은닉 상태(Hidden State)** 계산"), ("같은 방식으로 계속", "같은 가중치를 매 걸음 다시 쓰기")]),
    fig("펼쳐 본 RNN (N3 p.46)", SVG_RNN, "같은 Wh, We, U 를 걸음마다 다시 써요. 이것이 순환이에요", 3),
    form("RNN 두 줄 (N3 p.47)", r"h^{(t)} = \tanh\left(W_h h^{(t-1)} + W_e e^{(t)} + b\right),\qquad \hat{y}^{(t)} = \mathrm{softmax}(U h^{(t)})",
         [(r"h^{(t-1)}", "바로 앞까지의 **은닉 상태(Hidden State)**, 곧 예전 메모"),
          (r"W_h", "기억을 앞으로 옮기는 방법"),
          (r"W_e", "새 단어를 받아들이는 방법"),
          (r"\tanh", "값이 커지지 않게 눌러 담기"),
          (r"U", "**은닉 상태(Hidden State)** 에서 **어휘(Vocabulary)** 점수로")],
         "새 메모 = 예전 메모와 지금 단어를 섞은 것이에요. 같은 Wh, We, U 를 매 걸음 다시 써요."),
    steps("손계산: 50단어 문장에 쓰이는 행렬은 몇 개 (N3 p.56 Q3)", [
        "**순환 신경망(Recurrent Neural Network (RNN))** 은 걸음마다 같은 가중치를 써요",
        "쓰는 것은 Wh, We, U 세 가지예요",
        "50단어 문장이면 적용 횟수는 50 번이지만",
        "서로 다른 행렬의 개수는 여전히 3 개예요",
    ], "서로 다른 가중치 행렬은 3 개. 문장이 길어져도 늘지 않아요", "고정 윈도우는 창이 커질수록 W 가 커져요"),
    cmp_("고정 윈도우와 RNN (N3 p.48)", ["", "고정 윈도우", "**순환 신경망(Recurrent Neural Network (RNN))**"],
         [["문맥 길이", "창에 들어가는 만큼만", "원리상 제한이 없어요"],
          ["가중치", "자리마다 W 의 다른 조각", "Wh, We 하나를 모든 걸음이 공유"],
          ["모델 크기", "창이 커지면 커져요", "문장 길이와 상관없어요"],
          ["계산", "한 번에, 병렬", "차례대로. t 번째는 t-1 을 기다려요"],
          ["실제로는", "설계상 잊어버려요", "기울기가 흐려지며 잊어요"]]),
    pts("교수님이 짚은 곳", "매 위치에서 같은 **파라미터(Parameter)** 를 재사용해요",
        "그래서 문장이 길어져도 **파라미터(Parameter)** 수가 늘지 않아요",
        "다만 순서대로 계산해야 해서 느리고, 먼 기억은 흐려져요",
        "그 흐려짐이 다음 단원의 주제예요"),
    check("**순환 신경망(Recurrent Neural Network (RNN))** 이 문장 길이와 상관없이 모델 크기가 같은 까닭은?",
          ["걸음마다 같은 가중치를 다시 써서", "문장을 잘라서", "**임베딩(Embedding)** 이 작아서", "은닉 상태가 없어서"], 0,
          "Wh, We, U 를 모든 걸음이 공유해요. 그래서 **파라미터(Parameter)** 가 늘지 않아요."),
    check("**고정 윈도우 신경망 언어 모델(Fixed-window Neural LM)** 이 **엔그램(n-gram)** 보다 나은 점은?",
          ["**임베딩(Embedding)** 덕분에 비슷한 단어가 정보를 나눠요", "문맥을 무한히 봐요", "계산이 필요 없어요", "정답을 외워요"], 0,
          "**희소성 문제(Sparsity Problem)** 와 거대한 횟수 표가 사라져요. 다만 창은 여전히 고정이에요."),
    warn("헷갈리기 쉬운 점", "**은닉 상태(Hidden State)** h 는 w3-5 의 은닉층 값 h 와 기호가 같아요. 문맥으로 구별해요.",
         "RNN 이 허문 것은 고정 창이에요. **희소성 문제(Sparsity Problem)** 는 이미 **임베딩(Embedding)** 이 허물었어요.",
         "원리상 길이 제한이 없다는 것과 실제로 먼 기억을 잘 쓴다는 것은 다른 말이에요."),
    recap("**고정 윈도우 신경망 언어 모델(Fixed-window Neural LM)**: **임베딩(Embedding)** 으로 두 벽을 허물었지만 창이 고정",
          "**순환 신경망(Recurrent Neural Network (RNN))**: 같은 가중치 + **은닉 상태(Hidden State)** 로 어떤 길이든",
          "50단어 문장에도 서로 다른 행렬은 Wh, We, U 세 개뿐이에요",
          "오늘의 용어: 고정 윈도우 신경망 언어 모델, 임베딩, RNN, 은닉 상태, 파라미터, 희소성 문제"),
]})

# ---------------------------------------------------------------- w3-10
t = [T("시간 역전파", "Backpropagation Through Time (BPTT)", "시점마다 펼친 RNN 위에서 뒤에서 앞으로 역전파하는 것이에요.",
       "같은 Wh 가 모든 시점에 나오니 그 자리마다의 기울기를 모두 더해요. 2부의 '갈림길에서 더하기' 그대로예요."),
     T("기울기 소실", "Vanishing Gradient", "먼 곳에서 오는 기울기가 0 에 가까워져 배우지 못하는 문제예요."),
     T("기울기 폭발", "Exploding Gradient", "기울기가 걷잡을 수 없이 커져 학습이 튀는 문제예요."),
     T("기울기 클리핑", "Gradient Clipping", "기울기가 너무 길면 방향은 두고 길이만 기준까지 줄이는 것이에요."),
     T("장단기 메모리", "LSTM", "셀 상태라는 고속도로와 게이트를 더한 다음 세대 RNN 칸이에요."),
     T("게이트", "Gate", "무엇을 남기고, 쓰고, 내보낼지 정하는 여닫이예요."),
     T("교사 강요", "Teacher Forcing", "학습할 때 늘 진짜 앞 단어를 조건으로 넣어 주는 것이에요.")]
U.append({"id": "w3-10", "title": "BPTT, 기울기 소실과 폭발", "goal": "RNN 학습이 왜 어려운지 숫자로 보이고, 클리핑과 게이트의 역할을 구별할 수 있어요.", "terms": t, "slides": [
    title("멀리서 오는 신호", "BPTT, 소실, 폭발, 클리핑"),
    goal("RNN 언어 모델의 학습과 **시간 역전파(Backpropagation Through Time (BPTT))**", "**기울기 소실(Vanishing Gradient)** 과 **기울기 폭발(Exploding Gradient)**", "**기울기 클리핑(Gradient Clipping)** 이 고치는 것과 못 고치는 것"),
    pts("RNN 언어 모델 학습 (N3 p.49)", "말뭉치를 넣고 걸음마다 다음 단어를 맞혀요",
        "맞힐 때마다 정답 단어에 **교차 엔트로피(Cross-Entropy)** 벌점을 매겨요",
        "모든 걸음의 벌점을 평균 낸 것이 전체 손실 J 예요",
        "학습할 때는 늘 진짜 앞 단어를 조건으로 넣어요. 이것이 **교사 강요(Teacher Forcing)** 예요",
        "**교사 강요(Teacher Forcing)** 덕분에 학습이 안정되지만, 시험 때는 자기 출력만 보게 돼요"),
    form("걸음마다의 벌점과 평균 (N3 p.49)", r"J(\theta) = \frac{1}{T}\sum_{t=1}^{T} J^{(t)}(\theta),\qquad J^{(t)} = -\log \hat{y}^{(t)}\!\left[x^{(t+1)}\right]",
         [(r"\hat{y}^{(t)}", "t 번째 걸음에서 낸 **어휘(Vocabulary)** 확률"),
          (r"x^{(t+1)}", "진짜 다음 단어"),
          (r"-\log", "정답 확률이 작을수록 커지는 벌점"),
          (r"\frac{1}{T}\sum", "모든 걸음의 벌점 평균")],
         "w3-8 의 J 와 똑같은 식이에요. 그래서 exp 만 씌우면 **퍼플렉서티(Perplexity (PPL))** 가 나와요."),
    pts("같은 Wh 가 매 걸음 나와요 (N3 p.50)", "Wh 는 모든 시점에 한 번씩 등장해요",
        "2부에서 배웠죠. 갈라져 여러 곳에 쓰인 값은 되돌아올 때 기울기를 더해요",
        "그래서 Wh 의 기울기는 등장한 모든 자리의 기울기를 더한 것이에요",
        "펼친 그래프 위의 평범한 역전파, 이것이 **시간 역전파(Backpropagation Through Time (BPTT))** 예요"),
    fig("거슬러 갈수록 희미해져요 (N3 p.52)", SVG_VANISH, "걸음마다 야코비안을 곱하니 작은 수를 계속 곱하면 신호가 죽어요", 3),
    form("시간을 거슬러 가는 곱 (N3 p.52)", r"\frac{\partial h^{(t)}}{\partial h^{(1)}} = \prod_{i=2}^{t} \frac{\partial h^{(i)}}{\partial h^{(i-1)}}",
         [(r"\frac{\partial h^{(i)}}{\partial h^{(i-1)}}", "한 걸음짜리 **야코비안(Jacobian)**"),
          (r"\prod", "걸음 수만큼 곱하기"),
          (r"t", "지금 자리. 1 은 문장 맨 앞")],
         "작은 수를 여러 번 곱하면 0 으로 가고, 1 보다 큰 수를 여러 번 곱하면 폭발해요."),
    steps("손계산: 스무 걸음 거슬러 가면", [
        "한 걸음마다 대략 0.8 배가 된다고 해 봐요",
        "20 걸음: 0.8 의 20제곱 ≈ 0.0115",
        "곧 스무 단어 앞의 신호는 약 1% 로 줄어요. 이것이 **기울기 소실(Vanishing Gradient)** 이에요",
        "반대로 한 걸음마다 1.2 배라면 1.2 의 20제곱 ≈ 38.34",
        "이렇게 커지는 쪽이 **기울기 폭발(Exploding Gradient)** 이에요",
    ], "0.8 의 20제곱 ≈ 0.0115, 1.2 의 20제곱 ≈ 38.34", "0.8 과 1.2 는 느낌을 보기 위한 숫자예요"),
    ana("귓속말 전달 게임", "줄을 서서 귓속말을 전하면 뒤로 갈수록 말이 점점 희미해지고 결국 아무 말도 안 들려요.",
        [("맨 뒤에서 시작한 말", "출력 쪽에서 출발한 **기울기(Gradient)**"), ("한 사람을 거칠 때마다 흐려짐", "걸음마다 **야코비안(Jacobian)** 곱하기"), ("맨 앞에서는 안 들림", "**기울기 소실(Vanishing Gradient)**")]),
    pts("왜 중요한가 (N3 p.53)", "She bought extra toner for the ___ 에서 답 printer 는 20단어 앞에 있었어요",
        "20걸음 전 **기울기(Gradient)** 가 거의 0 이면 모델은 그 관계를 배울 수 없어요",
        "그러면 가까운 단어만 보고 찍게 돼요",
        "거울상 문제가 **기울기 폭발(Exploding Gradient)** 이에요. 한 번의 미친 걸음이나 Inf, NaN 이 나와요"),
    fig("방향은 두고 길이만 자르기 (N3 p.53)", SVG_CLIP, "**기울기 클리핑(Gradient Clipping)** 은 걸음의 방향을 지키고 크기만 줄여요", 3),
    steps("손계산: 클리핑 한 번 (N3 p.53)", [
        "**기울기(Gradient)** 가 g = (6, 8) 이라고 해요",
        "길이: 6 의 제곱 36 + 8 의 제곱 64 = 100, 제곱근 10",
        "기준 τ = 5 인데 10 이 더 크니 줄여야 해요",
        "g 를 길이 10 으로 나누면 (0.6, 0.8), 여기에 5 를 곱하면 (3, 4)",
        "확인: (3, 4) 의 길이는 3 의 제곱 9 + 4 의 제곱 16 = 25, 제곱근 5",
    ], "g = (6, 8) → (3, 4). 방향은 같고 길이만 10 에서 5 로 줄었어요", "식은 길이가 기준 τ 를 넘으면 g 를 τ 배 방향 벡터로 바꾸는 거예요"),
    pts("소실은 구조로 고쳐요 (N3 p.54)", "**장단기 메모리(LSTM)** 와 GRU 는 셀 상태라는 고속도로를 놓아요",
        "무엇을 남기고, 쓰고, 내보낼지 정하는 **게이트(Gate)** 를 배워요",
        "**기울기(Gradient)** 가 그 고속도로를 타고 멀리까지 살아남아요",
        "더 큰 처방은 다음 주예요. 역사를 벡터 하나에 밀어 넣지 말고 모든 걸음이 서로를 보게 하는 어텐션이에요"),
    pts("마무리와 과제 2 (N3 p.55, p.57)", "RNN 은 seq2seq 로 이어져요. 문장 넣고 문장 받기예요",
        "그 디코더가 바로 오늘 배운 조건부 **언어 모델(Language Model (LM))** 이에요",
        "Check Yourself 2: 못 본 5-gram 의 확률, PPL 40 대 25 비교, 50단어 문장의 행렬 수, 클리핑이 소실을 못 고치는 까닭",
        "과제 2 가 오늘 나와요(15%, N3 p.57 기준 W7). word2vec 기울기 손유도 + skip-gram 구현이에요"),
    check("**기울기 클리핑(Gradient Clipping)** 이 **기울기 소실(Vanishing Gradient)** 을 못 고치는 까닭은?",
          ["너무 작아진 기울기를 키워 주지는 않아서", "방향을 바꿔서", "계산이 느려서", "학습률을 바꿔서"], 0,
          "클리핑은 너무 큰 것을 줄일 뿐이에요. 0 에 가까운 신호는 그대로 0 에 가까워요."),
    check("한 걸음마다 0.8 배가 되는 신호는 20 걸음 뒤 약 얼마가 될까요?",
          ["약 0.0115", "약 0.8", "약 16", "약 38"], 0, "0.8 의 20제곱이에요. 이것이 **기울기 소실(Vanishing Gradient)** 이에요."),
    warn("헷갈리기 쉬운 점", "**기울기 클리핑(Gradient Clipping)** 은 폭발용 처방이에요. 소실은 **게이트(Gate)** 나 어텐션 같은 구조가 고쳐요.",
         "**시간 역전파(Backpropagation Through Time (BPTT))** 는 새 알고리즘이 아니라 펼친 그래프 위의 평범한 역전파예요.",
         "과제 2 마감은 슬라이드(W7)와 녹음(10월 7일)이 달라요. 공지를 꼭 확인해요."),
    recap("**시간 역전파(Backpropagation Through Time (BPTT))**: 같은 Wh 의 기울기를 모든 시점에서 더해요",
          "걸음마다 곱하다 보니 **기울기 소실(Vanishing Gradient)** 과 **기울기 폭발(Exploding Gradient)** 이 생겨요",
          "**기울기 클리핑(Gradient Clipping)** 은 폭발만, 소실은 **장단기 메모리(LSTM)** 의 **게이트(Gate)** 와 어텐션이 고쳐요",
          "오늘의 용어: 시간 역전파(BPTT), 기울기 소실, 기울기 폭발, 기울기 클리핑, LSTM, 게이트, 교사 강요"),
]})

d = {"week": "3", "title": "3주차. 신경망 NLP 의 기초", "units": U}
raw = json.dumps(d, ensure_ascii=False, indent=1)
for ch in ("—", "–", "·", "・"):
    assert ch not in raw, ch
(W / "slides_w3.json").write_text(raw, encoding="utf-8")
