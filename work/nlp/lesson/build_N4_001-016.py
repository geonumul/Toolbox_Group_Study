...
page(10, "디코더는 조건부 언어 모델", ["Conditional Language Model", "Language Model (LM)", "Cross-Entropy", "Loss Function", "End-to-End", "Backpropagation"],
     ...
     [formula("문장 확률을 쪼개는 식",
      ...
      points("3주차와 이어지는 점",
             ...
             f"{NLP} 전체가 이 한 가지 위에 서 있다고 봐도 돼요.",
             f"손실을 거슬러 보내는 것도 3주차의 {BPTT} 와 같은 방식이에요."),
      points("이 구조가 어디까지 가나",
             f"{S2S} 에 어텐션을 붙이고, RNN 을 빼면 트랜스포머가 돼요.",
             f"트랜스포머를 크게 쌓은 것이 {LLM} 이에요.",
             f"{NLP} 수업의 나머지가 전부 이 길 위에 있어요.",
             f"{BPTT} 는 RNN 을 쓰는 동안만 필요하고, 트랜스포머에서는 사라져요.")],
     [formula("학습 손실",
