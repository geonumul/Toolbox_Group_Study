...
page(9, "인코더 디코더 구조", ["Encoder", "Decoder", "Recurrent Neural Network (RNN)", "Hidden State", "Sequence-to-Sequence Model"],
     ...
     [check("인코더의 출력이 디코더로 넘어가는 통로는?",
      ...
      english("답안 문장",
              f"{S2S}은 {ENC} RNN 이 입력 문장을 읽어 고정 크기 벡터로 압축하고, {DEC} RNN 이 그 벡터를 조건으로 목표 문장을 한 토큰씩 생성하는 구조다.",
...
page(11, "학습 방법: 교사 강요", ["Teacher Forcing", "Exposure Bias", "Inference", "Cross-Entropy", "Loss Function"],
     ...
     [check("교사 강요의 대가로 생기는 문제는?",
      ...
      english("답안 문장",
              f"{TF}는 학습 시 디코더의 다음 입력으로 정답 토큰을 넣어 학습을 안정적이고 병렬적으로 만들지만, 추론 시에는 모델 자신의 출력만 보므로 학습과 추론 사이에 어긋남, 곧 {EB}이 생긴다.",
...
page(14, "번역 평가: BLEU", ["BLEU", "n-gram", "Brevity Penalty", "Machine Translation (MT)"],
     ...
     [check("BLEU 에서 4-gram 정밀도가 0 이면 어떻게 되나요?",
      ...
      english("답안 문장",
              f"{BLEU}는 모델 번역과 사람이 만든 정답 번역 사이에서 1-gram 부터 4-gram 까지의 겹침(정밀도)을 재고, 너무 짧은 번역을 막기 위해 {BP}를 곱하는 지표다. 뜻이 같아도 다른 표현을 쓰면 점수가 낮게 나오는 한계가 있다.",
...
page(15, "인코더 디코더는 번역만을 위한 것이 아니에요", ["Sequence-to-Sequence Model", "Encoder", "Decoder"],
     ...
     [check("다음 중 seq2seq 로 풀 수 있는 과제가 아닌 것은?",
      ...
      english("답안 문장",
              f"{S2S}은 특정 과제가 아니라 입력과 출력이 모두 시퀀스인 모든 문제에 쓰이는 {ENC}, {DEC} 프레임워크이며, 번역, 요약, 대화, 음성 인식, 코드 생성 등에 같은 구조가 재사용된다.",
...
page(16, "정보 병목", ["Information Bottleneck", "Encoder", "Decoder", "Hidden State", "Attention"],
     ...
     [exam("N4 Check Yourself",
      ...
      english("답안 문장",
              f"{S2S}에서 {ENC}의 정보는 고정 크기 벡터 하나를 통해서만 {DEC}로 전달되므로, 문장이 길어질수록 압축 손실이 커지는 {IB}이 생긴다.",
